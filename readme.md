...
### 5/29 Demo1.15 — Re 公式修正 + 干度模型升级

**Bug 修复：Re 计算缺少比容换算（根因）**

`R_cal_10.m` 中 Re 一直按 `Re = G·D/ν`（ν 为运动粘度）计算，但正确的定义是 `Re = G·D/μ = G·D/(ν/v)`，需除以比容 `v` 将运动粘度转为动力粘度。缺少该换算导致：
- 液相区 Re 虚高约 1000 倍 → `f` 过小 → 摩擦压降系统性偏小
- 气相区 Re 虚高约 10-50 倍
- `f` 改用分段 Fang 公式后（无需 Churchill 全范围拟合），对 Re 的敏感性降低

**改动细节：**

| 文件 | 修改 | 说明 |
|------|------|------|
| `R_cal_10.m:76-77` | `Re = G·D/(S·ν)` → `Re = G·D/(S·ν/v)` | ν 是运动粘度（mm²/s），除以比容 v 得动力粘度 |
| `Main.m:583-584` | `bend_cal` 内 Re 同样修正 | 弯管压降同步 |
| `R_cal_10.m:161` | `veloctiy_CV` → `velocity_CV` | 笔误 |
| `Main.m:112` | `h_R_tube_intlet` → `h_R_tube_inlet` | 笔误 |
| `buildPath.m:1` | 函数名 `buildPaths` → `buildPath` | 与文件名一致 |

**关联式变更：**

- **摩擦因子**：Churchill 全范围公式 → 三段 Fang 显式公式（层流 f=64/Re / 过渡区线性 / 湍流 Fang）
- **干度模型**：静态干度 `x_CV` → Zivi 滑移模型动态干度 `x_dyn`（`S=(ρₗ/ρ_g)^(1/3)`）
- **表面张力**：`sigma` 更新至匹配工质的值

**改动文件**：`R_cal_10.m`、`Main.m`、`Main_loop_base.m`、`buildPath.m`
...
#### C. Main_loop_base 性能对比 (2026-05-25)

`Main_loop_base.m` — 两阶段环路优先求解器：Phase1 仅热力扫描（恒压 + 全局饱和缓存）+ 提取压阻 → 流量稳定后进入 Phase2（完整热力+压力+流量更新，与 Main 一致）。

**16 管 4 环路对比测试结果：**

| 指标 | Main | Main_loop_base | 差异 |
|------|------|---------------|------|
| 耗时 | 0.26 s | 0.28 s | **+7%** (更慢) |
| 总换热量 | 2218.0 W | 2217.1 W | −0.04% |
| 集管总压降 | 4846.6 Pa | 4847.2 Pa | +0.01% |
| 外层迭代 | 4 | Phase1:2 + Phase2:1 | — |
| 收敛方式 | 4 iter 直接收敛 | Phase1 流量稳定 + Phase2 收敛 |

**关键发现：**

1. **精度无损失**：Phase1 的恒压假设和全局饱和缓存不引入系统性偏差——Phase2 用完整压力耦合将解修正回正确不动点。所有 CV 出口焓偏差 < 0.02%。
2. **速度未改善**：Phase1 虽省去压力扫描，但多了流量稳定所需的额外热力扫描迭代（2 次），且 Phase2 仍需至少 1 次完整迭代。总体扫描操作数相近，缓存优势被额外迭代抵消。
3. **待研究**：Phase1 步出判据 `delta_mdot < 1e-1`（10%）过松，导致 Phase1 过早退出、Phase2 接管后仍需大量修正。收紧阈值是否能减少总迭代数、能否自适应切换 Phase1/Phase2 权重是下一步研究方向。→ **5/26 已解决：收紧至 1e-2 后 loop_base 反超 Main 18%，见 §F。**
...
#### F. sat_cache 扩展到 11 项 + Prop1 句柄懒加载 (2026-05-26)

此前 `sat_cache` 缓存 9 项饱和物性（v, Pr, Nu, k 的液相/气相值 + Tsatliq），但 `hsatliq` 和 `hsatvap` 始终在 Prop1 中通过 `griddedInterpolant` 无条件计算——每次 Prop1 调用 2 次插值。

**改动：**

- `sat_cache` / `sat_in` / `sat_global` 统一从 9 项扩展到 11 项（新增 `{10}` hsatliq, `{11}` hsatvap）
- Prop1 有缓存时直接从 `sat_props{10}{11}` 取，不再调用 griddedInterpolant
- **Prop1 句柄懒加载**：缓存命中时，物性句柄（v_liq/v_vap/Pr_liq 等）推迟到相态分支内部提取——单相区只提取液/气一侧，两相区完全不需要提取（全部来自缓存）
- `R_cal_10` / `DryA_cal_10`：输出元胞数组增加 `out = cell(7)` 预分配，消除动态增长

**影响**：Prop1 每次调用节省 2 次 griddedInterpolant + 句柄提取开销（两相区省全部 10 个句柄提取）。5000+ 次调用级别下收益可观。

#### G. Phase1 步出阈值收紧 — loop_base 反超 Main 18% (2026-05-26)

此前 Phase1 步出判据 `delta_mdot < 1e-1`（10% 相对变化）过松，Phase1 过早退出导致 Phase2 接管后仍需大量修正，loop_base 反而比 Main 慢。

**收紧至 `1e-2`（1%）后的 40 管 6 环路对比：**

| 指标 | Main | Main_loop_base (1e-2) | 差异 |
|------|------|----------------------|------|
| 耗时 | 1.01 s | **0.83 s** | **−18%** |
| 总换热量 | 4124.2 W | 4124.1 W | −0.002% |
| 集管总压降 | 3955.4 Pa | 3955.6 Pa | +0.005% |
| 外层迭代 | 7 | Phase1:? + Phase2:? | — |
| Phase1 退出 delta_mdot | — | 6.17e-03 | — |

**对比 1e-1 阈值（16 管）时的结论：**
...
**结论**：收紧阈值让 Phase1 多做了有效的流量稳定工作，Phase2 获得更好的初值从而总时间更短。Phase1 的 "额外迭代" 并非浪费——它是用廉价的热力扫描迭代替代 Phase2 中昂贵的"热力+压力"完整扫描。自适应阈值（如根据环路数/管数自动选择 1e-2 或 1e-3）是下一步方向。

### ⚠️ 重要警告：流量分配与压降符号

管压降 $dp_{tube} = dp_{friction} + dp_{velocity}$（摩擦项 + 速度头项）。当某根管的流量过小时，摩擦压降 $dp_f \propto \dot{m}^2$ 可小至个位数 Pa，而速度压损 $dp_v = 16\dot{m}^2/(\pi^2 D^4) \cdot (1/\rho_{out} - 1/\rho_{in})$ 反而因密度变化（冷凝段液→气膨胀）成为主导，导致总压降**为正数**（即管出口压力高于进口）。一旦出现正压降，$R_{flow}$ 变负，流量重分配 Newton 迭代直接发散。

**流量分配的根源于压损。压损一旦为负，就会导致错误的流量分配。**

**排查口诀**：如果 `dp_tube` 为负 → 检查该管 `mdot` 是否过小 → 调整 `mdot_Initial` 的 `epsilon` 下界（`mdot_R_inlet/4` → 更紧的下界）。
